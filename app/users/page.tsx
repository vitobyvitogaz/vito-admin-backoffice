    ...
    try {
      await fetch(`${API_URL}/users/${id}/reset-password`, {
        ...
        headers: {
          ...
          'Authorization': `Bearer ${localStorage.getItem('vito_auth_token')}`,
        },
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="p-6">
        ...
        <div className="flex items-center justify-between mb-6">
          ...
          <Button onClick={() => { setShowForm(!showForm); setShowResetForm(null); }} className="gap-2 text-white" style={{ backgroundColor: VITOGAZ_GREEN }}>
          ...
          <CardContent className="pt-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3 text-sm">
              <div className="flex items-start gap-2">
                ...
                <div><div className="font-semibold text-red-700">SUPER_ADMIN</div><div className="text-gray-600">Accès total + gestion utilisateurs</div></div>
              </div>
              <div className="flex items-start gap-2">
                <Shield className="w-4 h-4 text-orange-600 mt-0.5" />
                <div><div className="font-semibold text-orange-700">ADMIN</div><div className="text-gray-600">CRUD sur toutes ressources</div></div>
              </div>
              <div className="flex items-start gap-2">
                <KeyRound className="w-4 h-4 text-blue-600 mt-0.5" />
                <div><div className="font-semibold text-blue-700">EDITOR</div><div className="text-gray-600">Modification limitée</div></div>
              </div>
              <div className="flex items-start gap-2">
                <Eye className="w-4 h-4 text-emerald-600 mt-0.5" />
                <div><div className="font-semibold text-emerald-700">VIEWER</div><div className="text-gray-600">Lecture seule</div></div>
              </div>
              <div className="flex items-start gap-2">
                <User className="w-4 h-4 text-purple-600 mt-0.5" />
                <div><div className="font-semibold text-purple-700">API_CLIENT</div><div className="text-gray-600">Accès API limité</div></div>
              </div>
            </div>
          </CardContent>
        </Card>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  {editingId && (
                    <div className="md:col-span-2">
                      <p className="text-sm text-gray-500 bg-gray-50 px-3 py-2 rounded-md">
                        Email : <strong>{formData.email}</strong> (non modifiable)
                      </p>
                    </div>
                  )}
                  <div>
                    <Label htmlFor="first_name">Prénom</Label>
                    <Input id="first_name" value={formData.first_name} onChange={(e) => setFormData({ ...formData, first_name: e.target.value })} placeholder="Jean" />
                  </div>
                  <div>
                    <Label htmlFor="last_name">Nom</Label>
                    <Input id="last_name" value={formData.last_name} onChange={(e) => setFormData({ ...formData, last_name: e.target.value })} placeholder="Rakoto" />
                  </div>
                  <div>
                    <Label htmlFor="role">Rôle *</Label>
                    <select id="role" className="flex h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-sm" value={formData.role} onChange={(e) => setFormData({ ...formData, role: e.target.value })}>
                  ...
                  {!editingId && (
                    <div>
                      ...
                      <Input id="password" type="password" required={!editingId} value={formData.password} onChange={(e) => setFormData({ ...formData, password: e.target.value })} placeholder="Minimum 8 caractères" />
                    </div>
                  )}
                </div>
                {!editingId && (
        ...
        {/* Formulaire reset password inline */}
        {showResetForm && (
          <Card className="mb-6 border-amber-200">
            <CardHeader>
              <CardTitle className="text-base text-amber-800">🔑 Réinitialiser le mot de passe</CardTitle>
            </CardHeader>
            <CardContent>
              <div className="flex items-center gap-3">
                <Input
                ...
                <Button
                  ...
                  disabled={resettingId === showResetForm}
                ...
                <Button variant="outline" onClick={() => { setShowResetForm(null); setNewPassword(""); }}>
                  Annuler
                </Button>
              </div>
            </CardContent>
        ...
        <Card className="mb-6">
          <CardContent className="pt-6">
            <div className="relative">
              ...
              <Input placeholder="Rechercher par email, nom ou rôle..." className="pl-10" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} />
            </div>
          </CardContent>
        </Card>
        ...
        <Card>
          <Table>
            ...
            <TableBody>
              {loading ? (
                ...
                <TableRow><TableCell colSpan={6} className="text-center py-8">Aucun utilisateur trouvé</TableCell></TableRow>
              ) : (
                  ...
                  <TableRow key={user.id}>
                    <TableCell>
                      <div className="flex flex-col">
                        ...
                        {getDisplayName(user) && (
                          <span className="text-xs text-gray-400">{getDisplayName(user)}</span>
                        )}
                      </div>
                    </TableCell>
                    <TableCell>
          ...
          <div className="flex items-center justify-between px-4 py-3 border-t border-gray-200 min-h-[52px]">
            <p className="text-xs text-gray-400 italic">Connecté à l'API — données en temps réel</p>
