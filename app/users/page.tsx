...
import {
  ...
  User, Eye, KeyRound, RefreshCw, Lock,
} from "lucide-react";
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="p-6">
        ...
        <div className="flex items-center justify-between mb-6">
          ...
          {/* Bouton "Nouvel Utilisateur" — SUPER_ADMIN uniquement */}
          {canManageUsers && (
            <Button
              onClick={() => { setShowForm(!showForm); setShowResetForm(null); }}
        ...
        {/* Bandeau lecture seule pour les non-SUPER_ADMIN */}
        {!canManageUsers && (
        ...
        {/* Formulaire création/modification — SUPER_ADMIN uniquement */}
            ...
            <CardContent>
              <form onSubmit={handleSubmit} className="space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  {!editingId && (
                    <div>
                      ...
                      <Input id="password" type="password" required value={formData.password} onChange={(e) => setFormData({ ...formData, password: e.target.value })} placeholder="Minimum 8 caractères" />
                    </div>
                  )}
                </div>
                {!editingId && (
                ...
                <div className="flex gap-3">
                  <Button type="submit" className="text-white" style={{ backgroundColor: VITOGAZ_GREEN }}>{editingId ? "Mettre à jour" : "Créer"}</Button>
        ...
        {/* Formulaire reset password inline — SUPER_ADMIN uniquement */}
        {showResetForm && canManageUsers && (
          <Card className="mb-6 border-amber-200">
            ...
            <CardContent>
              <div className="flex items-center gap-3">
                <Input type="password" placeholder="Nouveau mot de passe (min. 8 caractères)" value={newPassword} onChange={(e) => setNewPassword(e.target.value)} className="flex-1" />
                <Button onClick={() => handleResetPassword(showResetForm)} disabled={resettingId === showResetForm} className="text-white gap-2" style={{ backgroundColor: VITOGAZ_GREEN }}>
          ...
          <Table>
            <TableHeader>
              <TableRow>
                ...
                {/* Colonne Actions uniquement pour SUPER_ADMIN */}
                    ...
                    {canManageUsers && (
                      <TableCell className="text-right">
                        <div className="flex justify-end gap-2">
                          <Button variant="outline" size="sm" onClick={() => { setShowResetForm(showResetForm === user.id ? null : user.id); setNewPassword(""); setShowForm(false); }} title="Réinitialiser le mot de passe">
                            <RefreshCw className="w-4 h-4" />
                          </Button>
